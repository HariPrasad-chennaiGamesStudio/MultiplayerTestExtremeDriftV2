...
public class VehicleControl : MonoBehaviour
{
    ...
    // public void ChangeMaterialPropertyToMatteOrGlossy(float _smoothness, float _matte)
    // {
    //     carBody_Mat.SetFloat("_Glossiness",_smoothness);
    //     carBody_Mat.SetFloat("_Mettallic",_matte);
    // }

    // public void SetSpecularTrue()
    // {
    //     carBody_Mat.DisableKeyword("_SPECULARHIGHLIGHTS_OFF");
    //     carBody_Mat.SetFloat("_SpecularHighlights",1f);    
    // }

    // public void SetSpecularFalse()
    // {
    //     carBody_Mat.EnableKeyword("_SPECULARHIGHLIGHTS_OFF");
    //     carBody_Mat.SetFloat("_SpecularHighlights",0f);
    // }
    ...
    private void Start() 
    {
       if(NetworkClient.instance.otherPlayersSpawning == false)
        {
            AIControl.CurrentVehicle = this;//instantiatedCar.GetComponent<VehicleControl>();

            VehicleCamera.manage.target = this.transform;
            VehicleCamera.manage.cameraSwitchView = AIControl.CurrentVehicle.cameraView.cameraSwitchView;

            VehicleCamera.manage.distance = AIControl.CurrentVehicle.cameraView.distance;
            VehicleCamera.manage.height = AIControl.CurrentVehicle.cameraView.height;
            VehicleCamera.manage.angle = AIControl.CurrentVehicle.cameraView.angle;
        } 
    }
    ...
    void Awake()
    {
        ...
        Debug.Log("Vehicle control return not working........");
    ...
    void Update()
    {
        ...
        if (!isMultiplayerAI)
        {
            ...
            if (lastSyncedTime + 0.1f < Time.time)
            {
    ...
    void FixedUpdate()
    {
        ...
        if (carSetting.carSteer)
            ...
            // Debug.Log("carSetting.carSteer.localEulerAngles :: "+carSetting.carSteer.localEulerAngles);
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if(wheelId == 2 || wheelId == 0)
            {
                if(wheelId == 0)
                {
                    w.wheel.localRotation = Quaternion.Euler(w.rotation, 180.0f + w.rotation2, 0.0f);
                }else
                {
                    w.wheel.localRotation = Quaternion.Euler(w.rotation, 180.0f, 0.0f);
                }
            }else
            {
